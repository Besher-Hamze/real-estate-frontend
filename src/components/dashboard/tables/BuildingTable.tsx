...
import { Edit2, Trash2, X, Home, QrCode, ArrowLeft, Plus, Building2, ExternalLink } from "lucide-react";
...
export default function BuildingsTable() {
  ...
  const {
    ...
    queryFn: () => selectedBuilding ? RealEstateApi.fetchRealEstateByBuildingItemId(selectedBuilding.id) : Promise.resolve([]),
  ...
  // Enhanced function to navigate to real estate creation page
  const handleAddRealEstate = (building: Building) => {
    // Navigate to the real estate creation page with building ID
    router.push(`/dashboard/real-estate/create?buildingId=${building.id}`);
  };

  // Alternative: If you want to keep the modal approach, use this instead
  const handleAddRealEstateModal = (building: Building) => {
    setSelectedBuilding(building);
      ...
      <div className="w-full" dir="rtl">
        ...
        <div className="bg-white rounded-lg p-4 mb-6 flex items-center justify-between">
          <div className="flex items-center gap-6">
            ...
            <div>
              <span className="text-gray-600">حالة المبنى:</span>
              <span className={`inline-flex items-center px-2.5 py-1 rounded-full text-xs font-medium mr-2 ${selectedBuilding.status === 'مكتمل' ? 'bg-green-100 text-green-800' :
                selectedBuilding.status === 'قيد الإنشاء' ? 'bg-yellow-100 text-yellow-800' :
            ...
            {selectedBuilding.location && (
              <div>
                ...
                <span className="text-sm text-gray-500 mr-2">{selectedBuilding.location}</span>
              </div>
            )}
          </div>
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-4">
            <div className="text-center">
              <p className="text-2xl font-bold text-blue-600">{buildings?.length || 0}</p>
            ...
            >
              ...
              <div className="text-center">
                ...
                <div className="flex justify-center mb-4">
                  <canvas ref={qrCanvasRef} className="w-48 h-48 border rounded-lg" />
                </div>
                <p className="text-sm text-gray-600 mt-2 mb-4">
                  امسح الرمز للوصول إلى صفحة المبنى: {selectedBuilding.title}
                </p>
